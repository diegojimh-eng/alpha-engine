...
=============================
Adapter layer between the web layer and the valuation engine.

RULE: This file ONLY transforms data shapes.
      It NEVER modifies any financial value.

Flow:
  ValuationRequest (Pydantic, percent-normalised)
      → DCFInputs (dataclass, decimal)
      → calculate_dcf()
      → DCFResult (dataclass)
      → ValuationResponse (Pydantic, serialisable)
"""
...
# Make valuation_engine importable regardless of working directory
...
def request_to_inputs(req: ValuationRequest) -> DCFInputs:
    """
    Map a validated ValuationRequest directly to DCFInputs.

    The pct_to_decimal validator in ValuationRequest has already
    converted all percent fields to decimals, so we pass them through
    unchanged.
    """
    return DCFInputs(
...
def result_to_response(result: DCFResult) -> ValuationResponse:
    """
    Map a DCFResult (from the engine) to a ValuationResponse (for the API).

    All numeric values are passed through without modification.
    """
    ...
    return ValuationResponse(
        ...
        # Chart data — straight pass-through from engine output
...
def run_valuation(req: ValuationRequest) -> ValuationResponse:
    ...
    Orchestrates: request → inputs → engine → response.
    Nothing else.
    """
    inputs = request_to_inputs(req)
    result = calculate_dcf(inputs)          # ← THE ONLY CALL TO THE ENGINE
